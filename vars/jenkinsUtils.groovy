 ...
 * @return
 */
def emaiBuildDev(status) {
    def lastFailedBefore = getLastSuccessfulBefore();
    def lastSucessBefore = getLastFailedBefore();
    def buildLog = getBuildLog();
    def subject = "Build ${status} in Jenkins: ${env.JOB_NAME} #${env.BUILD_NUMBER}"
    def bodyText = "Build Number: ${env.BUILD_NUMBER} <br>" +
    ...
    emailDev(subject, bodyText);
}

/**
 * Sends email to "developers@level.systems".
 * @param subject
 * @param bodyText
 * @return
 */
def emailDev(subject, bodyText) {
    email(subject, bodyText, "developers@level.systems");
}
 ...
 */
@NonCPS
def convertMillisToString(millis) {
